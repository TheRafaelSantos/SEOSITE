	...
	public function render() {
		if ( is_admin() && ( static::is_error() || empty( static::get_appID() ) ) ) {
			echo '<div class="elementor-alert elementor-alert-danger">';
				...
				if ( empty( static::get_appID() ) ) {
					echo esc_html__( 'Please go to the ' , 'cmsmasters-elementor' ) . '<a href="' . esc_url( admin_url( 'admin.php?page=cmsmasters-addon-settings' ) ) . '" target="_blank">' . __( 'settings page', 'cmsmasters-elementor' ) . '</a>' . esc_html__( ' and add your Open Weather api key' , 'cmsmasters-elementor' );
				} elseif ( static::is_error() ) {
					$weather = static::get_weather();
