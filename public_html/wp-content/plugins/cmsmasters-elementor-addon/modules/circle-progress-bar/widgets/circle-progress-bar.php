...
class Circle_Progress_Bar extends Base_Widget {
		...
		if ( ! empty( $settings['progress_bars'] ) ) {
			...
			foreach ( $settings['progress_bars'] as $index => $progress_bar ) {
				...
				$options = [
					...
					'circle' => $settings['circle'] == 'yes' ? false : true,
    ...
    public function get_progress_icon( $progress_bar ) {
        if ( ! empty( $progress_bar['icon'] ) ) {
            Utils::render_icon( $progress_bar['icon'], array( 'aria-hidden' => 'true' ), true );
        }
    }
    ...
    public function get_progress_title( $progress_bar ) {
        ...
        $title = ( ! empty( $progress_bar['title'] ) ? $progress_bar['title'] : __( 'Progress Title', 'cmsmasters-elementor' ) );

        echo "<{$title_tag} class='cmsmasters-circle-progress-bar__title'>{$title}</{$title_tag}>"; 
    }
    ...
    public function get_progress_value( $progress_bar ) {
        ...
        echo "<span class='cmsmasters-circle-progress-bar__wrapper-value'>";
            if ( ! empty( $value_prefix ) ) {
                echo "<span class='cmsmasters-circle-progress-bar__prefix-value'>{$value_prefix}</span>";
            }

            echo "<span class='cmsmasters-circle-progress-bar__value'>{$value}</span>";

            if ( ! empty( $value_suffix ) ) {
                echo "<span class='cmsmasters-circle-progress-bar__suffix-value'>{$value_suffix}</span>";
            }
        echo "</span>";
    }
