...
abstract class Base_Events extends Base_Widget {
	...
	*/
	...
	public function render_event_open() {
		$event_class = 'cmsmasters-tribe-events__event';

		$this->add_render_attribute(
			'event',
			array(
				'id' => 'event-' . get_the_ID(),
				'class' => get_post_class( $event_class ),
			),
			null,
			true
		);
		...
		echo '<article ' . $this->get_render_attribute_string( 'event' ) . '>' .
			'<div class="' . $event_class . '-cont-wrap">';
	}
	...
	protected function render() {
		...
		if ( ! $wp_query->found_posts ) {
			if ( $this->is_current_query() ) {
				...
				Utils::render_alert( esc_html__( 'Events not found!', 'cmsmasters-elementor' ) );
			}
