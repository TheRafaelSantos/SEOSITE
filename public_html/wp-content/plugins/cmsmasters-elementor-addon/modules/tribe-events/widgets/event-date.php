...
class Event_Date extends Base_Widget {
	...
	protected function get_date( $switcher, $date, $period ) {
		...
		if ( 'start' === $switcher || 'end' === $switcher ) {
			echo '<div class="' . $this->get_widget_class() . '__' . $switcher . '-date-first cmsmasters-first-date">' .
				date( $this->get_format( 'first' ), strtotime( $date ) ) .
			'</div>';

			$event_date_additional = ( isset( $settings['event_date_additional'] ) ? $settings['event_date_additional'] : '' );

			if ( 'none' !== $event_date_additional ) {
				echo '<div class="' . $this->get_widget_class() . '__' . $switcher . '-date-additional cmsmasters-additional_date">' .
					date( $this->get_format( 'additional' ), strtotime( $date ) ) .
				'</div>';
			}
			...
			foreach ( $items as $item => $value ) {
				if ( $interval->$item > 0 ) {
					echo '<div class="' . $this->get_widget_class() . '__' . $switcher . '-' . $value . '">' .
						$interval->format( '%' . $item . ' ' . $value . ( $last_key !== $item && $coma ? ',' : '' ) ) .
					'</div>';
				}
			}
		}
