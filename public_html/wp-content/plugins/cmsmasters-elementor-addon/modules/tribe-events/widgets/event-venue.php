...
class Event_Venue extends Base_Widget {
	...
	protected function register_customize_data_controls_style() {
		foreach ( $this->get_venue_options() as $data => $value ) {
			...
			$this->start_controls_section(
				...
				array(
					'label' => esc_html__( "Customize: {$value}", 'cmsmasters-elementor' ),
					'tab' => Controls_Manager::TAB_STYLE,
					'condition' => $condition,
				)
			);
	...
	protected function get_venue_data( $data ) {
		...
		if ( ! empty( $value ) ) {
			echo '<div class="' . $this->get_widget_class() . '__' . $data . '">';
			...
			if ( 'venue-title' === $data ) {
				...
				echo '<a href="' . esc_url( $venue_website ) . '">';
				...
				if ( 'icon' === $venue_website_view ) {
					Icons_Manager::render_icon( $settings['venue_website_icon'] );
				}
				...
				echo '<a href="' . esc_url( $venue_map_url ) . '">';
					Icons_Manager::render_icon( $settings['venue_map_icon'] );
				echo '</a>';
			} elseif ( 'venue-phone' === $data ) {
				$venue_phone = ( isset( $settings['venue_phone'] ) ? $settings['venue_phone'] : '' );

				echo '<a href="tel:' . esc_attr( $venue_phone ) . '">';
				...
				if ( 'icon' === $venue_phone_view ) {
					Icons_Manager::render_icon( $settings['venue_phone_icon'] );
				}
