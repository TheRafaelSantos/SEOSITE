	...
	protected function get_organizer_data( $data ) {
		...
		if ( ! empty( $value ) ) {
			...
			if ( 'organizer-title' === $data ) {
				...
				$organizer_data = ( isset( $settings[ $settings_id ] ) ? $settings[ $settings_id ] : '' );

				echo '<a href="' . $link . esc_attr( $organizer_data ) . '">';
				...
				if ( 'icon' === $organizer_data_view ) {
					Icons_Manager::render_icon( $settings[ "{$settings_id}_icon" ] );
				}
