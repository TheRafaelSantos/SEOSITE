...
class Product_Add_To_Cart extends Base_Widget {
	...
	public function custom_add_to_cart_button() {
		...
		echo '<button type="submit" class="cmsmasters-add-to-cart-button single_add_to_cart_button button alt ' . $product_type . esc_attr( wc_wp_theme_get_element_class_name( 'button' ) ? ' ' . wc_wp_theme_get_element_class_name( 'button' ) : '' ) . '"' . ( 'external' !== $product_type ? ' data-product_id="' . $product_id . '"' : '' ) . '>' .
