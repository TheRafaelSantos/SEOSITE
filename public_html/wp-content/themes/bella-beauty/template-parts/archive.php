...
use BellaBeautySpace\TemplateFunctions\Post_Elements;
...
if ( is_search() && ! have_posts() ) {
	echo '<div class="' . $parent_class . '__no-posts">' .
		'<h3 class="' . $parent_class . '__no-posts-title">' . esc_html__( 'Nothing found', 'bella-beauty' ) . '</h3>' .
		'<div class="' . $parent_class . '__no-posts-description">' .
			'<p>' . esc_html__( 'Sorry, no posts matched your criteria. Please try another search.', 'bella-beauty' ) . '</p>' .
			'<p>' . esc_html__( 'You might want to consider some of our suggestions to get better results:', 'bella-beauty' ) . '</p>' .
			'<ul>' .
				'<li>' . esc_html__( 'Check your spelling.', 'bella-beauty' ) . '</li>' .
				'<li>' . esc_html__( 'Try a similar keyword, for example: tablet instead of laptop.', 'bella-beauty' ) . '</li>' .
				'<li>' . esc_html__( 'Try using more than one keyword.', 'bella-beauty' ) . '</li>' .
			'</ul>' .
		'</div>' .
	'</div>';
}
