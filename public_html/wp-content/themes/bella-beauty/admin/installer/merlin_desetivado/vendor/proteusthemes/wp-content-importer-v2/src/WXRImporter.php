...
class WXRImporter extends \WP_Importer {
	...
	protected function process_post( $data, $meta, $comments, $terms ) {
		...
		if ( 'attachment' === $postdata['post_type'] ) {
			...
			$cmsmasters_imported_attachments_ids = get_option( "cmsmasters_bella-beauty_{$cmsmasters_demo}_import_attachments_ids" );

			if ( isset( $cmsmasters_imported_attachments_ids[ $original_id ] ) ) {
				$this->logger->notice( sprintf(
					__( 'Skipping attachment "%s", this attachment already imported' ),
					$data['post_title']
				) );
			...
			$cmsmasters_imported_posts_ids = get_option( "cmsmasters_bella-beauty_{$cmsmasters_demo}_import_displayed_ids" );

			if ( isset( $cmsmasters_imported_posts_ids[ $original_id ] ) ) {
				$this->logger->notice( sprintf(
					__( 'Skipping post "%s", this post already imported' ),
					$data['post_title']
				) );
