...
class Elementor_Fonts {
	...
	public static function set_import_status( $status = 'pending' ) {
		...
		if ( 'done' === get_option( "cmsmasters_bella-beauty_{$demo}_elementor_fonts_import" ) ) {
			return;
		}

		update_option( "cmsmasters_bella-beauty_{$demo}_elementor_fonts_import", $status );
	}
	...
	public function get_local_fonts() {
		...
		if ( 'pending' !== get_option( "cmsmasters_bella-beauty_{$demo}_elementor_fonts_import", 'done' ) ) {
			return;
		}
		...
		update_option( "cmsmasters_bella-beauty_{$demo}_elementor_fonts_import", 'done' );
	}
	...
	private function import_single_font( $file_path ) {
		$post_id = wp_insert_post( wp_slash( array(
			...
			'post_title' => esc_html__( '(no title)', 'bella-beauty' ),
			'post_status' => 'draft',
		) ), true );
	...
	public function local_font_upload_handler( $data ) {
		...
		if ( empty( $config ) ) {
			return new \WP_Error( 'unsupported_zip_format', esc_html__( 'The zip file provided is not supported!', 'bella-beauty' ) );
		}
