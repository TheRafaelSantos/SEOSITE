...
class WPClever_Importer_Base {
	...
	public static function get_import_status( $default = 'done' ) {
		return get_option( 'cmsmasters_bella-beauty_' . Utils::get_demo() . '_wpclever_' . self::MODULE_NAME . '_import', $default );
	}
	...
	public static function set_import_status( $status = 'pending' ) {
		...
		update_option( 'cmsmasters_bella-beauty_' . Utils::get_demo() . '_wpclever_' . self::MODULE_NAME . '_import', $status );
	}
	...
	protected function set_exists_options() {
		$this->options = get_option( 'cmsmasters_bella-beauty_' . Utils::get_demo() . '_wpclever_' . self::MODULE_NAME, array() );
	}
	...
	public static function set_backup_options( $first_install = false ) {
		...
		$option_name = 'cmsmasters_bella-beauty_' . Utils::get_demo() . '_wpclever_' . self::MODULE_NAME;

		if ( $first_install ) {
			$option_name = 'cmsmasters_bella-beauty_wpclever_' . self::MODULE_NAME . '_backup';
		}
