...
class Forminator {
	...
	public static function get_import_status( $default = 'done' ) {
		return get_option( 'cmsmasters_bella-beauty_' . Utils::get_demo() . '_forminator_import', $default );
	}
	...
	public static function set_import_status( $status = 'pending' ) {
		...
		update_option( 'cmsmasters_bella-beauty_' . Utils::get_demo() . '_forminator_import', $status );
	}
	...
	protected function import_forms() {
		$data = API_Requests::post_request( 'get-forminator', array( 'demo' => Utils::get_demo() ) );
		...
		foreach ( $data as $type => $forms ) {
			...
			foreach ( $forms as $form ) {
				...
				if ( $new_id && isset( $original_form['post_id'] ) ) {
					...
					$forms_ids = get_option( "cmsmasters_bella-beauty_{$demo}_forminator_import_forms_ids" );
					...
					update_option( "cmsmasters_bella-beauty_{$demo}_forminator_import_forms_ids", $forms_ids, false );
				}
			}
		}
	}
