...
class Elementor_Attachments {
	...
	private static function replace_attachment_data( $settings, $attachments_ids ) {
		if (
			isset( $settings['library'] ) &&
			'svg' === $settings['library'] &&
			! empty( $settings['value']['id'] ) &&
			isset( $settings['value']['url'] )
		) {
			$svg_id = $settings['value']['id'];
			$svg_url = $settings['value']['url'];

			if ( isset( $attachments_ids[ $svg_id ] ) ) {
				$svg_id = $attachments_ids[ $svg_id ];
			...
			$settings['value']['id'] = $svg_id;
			$settings['value']['url'] = $svg_url;
