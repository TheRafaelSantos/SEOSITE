...
class WPForms {
	...
	public static function get_import_status( $default = 'done' ) {
		return get_option( 'cmsmasters_bella-beauty_' . Utils::get_demo() . '_wpforms_import', $default );
	}
	...
	public static function set_import_status( $status = 'pending' ) {
		...
		update_option( 'cmsmasters_bella-beauty_' . Utils::get_demo() . '_wpforms_import', $status );
	}
	...
	protected function import_forms() {
		$data = API_Requests::post_request( 'get-wpforms', array( 'demo' => Utils::get_demo() ) );
		...
		foreach ( $forms as $form ) {
			...
			if ( $new_id ) {
				...
				$forms_ids = get_option( "cmsmasters_bella-beauty_{$demo}_wpforms_import_forms_ids" );
				...
				update_option( "cmsmasters_bella-beauty_{$demo}_wpforms_import_forms_ids", $forms_ids, false );
