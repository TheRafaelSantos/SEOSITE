<?php
namespace BellaBeautySpace\Admin;

use BellaBeautySpace\Admin\Installer\Installer; // A classe ainda é referenciada aqui, mas não será instanciada abaixo
use BellaBeautySpace\Admin\Options\Options_Manager;
use BellaBeautySpace\Admin\Upgrader\Upgrader;
...
class Admin {
	...
	public function __construct() {
		...
		// new Installer(); // <--- LINHA COMENTADA PARA DESATIVAR O INSTALADOR (MERLIN WP)
	...
	public function enqueue_admin_assets() {
		...
		wp_localize_script( 'bella-beauty-admin', 'cmsmasters_admin', array(
	...
	public function ajax_hide_admin_notice() {
		if ( ! check_ajax_referer( 'cmsmasters_admin_nonce', 'nonce' ) ) {
			wp_send_json( array(
				'success' => false,
				'code' => 'invalid_nonce',
				'message' => esc_html__( 'Invalid nonce. Notice was not deleted.', 'bella-beauty' ),
			) );
		}
		...
		if ( ! isset( $_POST['option_key'] ) ) {
			wp_send_json( array(
				...
				'message' => esc_html__( 'Empty option key.', 'bella-beauty' ),
			) );
		}
	...
	public function elementor_activation_notice() {
		...
		if ( isset( $plugins['elementor/elementor.php'] ) ) {
			...
			$link_text = esc_html__( 'Activate', 'bella-beauty' );
		} else {
			$link_url = wp_nonce_url( self_admin_url( 'update.php?action=install-plugin&plugin=elementor' ), 'install-plugin_elementor' );
			$link_text = esc_html__( 'Install', 'bella-beauty' );
		}

		echo '<div class="notice notice-error">' .
			'<p>' .
				sprintf(
					esc_html__( '%s requires Elementor to be activate.', 'bella-beauty' ),
					'<strong>' . esc_html__( 'The Theme', 'bella-beauty' ) . '</strong>'
				) .
	...
	public function license_activation_notice() {
		...
		echo '<div class="notice notice-warning is-dismissible">' .
			'<p><strong>' . esc_html__( 'Your license is not activated.', 'bella-beauty' ) . '</strong></p>' .
			'<p>' .
				sprintf(
					esc_html__(
					...
					'<strong><a href="' . esc_url( self_admin_url( 'admin.php?page=cmsmasters-options-license' ) ) . '">' . esc_html__( 'activate the license', 'bella-beauty' ) . '</a></strong>'
				) .
			'</p>' .
		'</div>';
	}
		...
		echo '<div class="notice notice-info is-dismissible cmsmasters-dismiss-notice-permanent" data-option-key="cmsmasters_license_deactivation_notice_visibility">' .
			'<p>' .
				sprintf(
					...
					'<a href="' . esc_url( self_admin_url( 'admin.php?page=cmsmasters-options-license' ) ) . '">' . esc_html__( 'deactivate theme license', 'bella-beauty' ) . '</a>'
				) .
			'</p>' .
		'</div>';
	}
	...
	public function apply_demo_notice() {
		...
		echo '<div class="notice notice-info is-dismissible cmsmasters-dismiss-notice-permanent" data-option-key="cmsmasters_apply_demo_notice_visibility">' .
			'<p>' .
				sprintf(
					__( 'You have applied a new design concept to your website. Image sizes in design concepts may differ, this is why it is recommended to run a %1$sRegenerate Thumbnails%2$s tool to generate new image sizes.', 'bella-beauty' ),
	...
	public function invalid_admin_email_notice() {
		...
		echo '<div class="notice notice-warning is-dismissible">' .
			'<p>' .
				sprintf(
					__( 'Oops, looks like you tried to use our email: %s', 'bella-beauty' ),
					'<strong>' . $current_user->user_email . '</strong>'
				) .
			'</p>' .
			'<p>' . esc_html__( 'Please enter yours instead.', 'bella-beauty' ) . '</p>' .
		'</div>';
	}
	...
	public function export_theme_options_name() {
		return 'cmsmasters_bella-beauty_options';
	}
