...
class Demos extends Base\Base_Page {
	...
	public static function get_page_title() {
		return esc_attr__( 'Designs', 'bella-beauty' );
	}
		...
		echo '<div class="' . esc_attr( $parent_class ) . '">' .
			'<div class="' . esc_attr( $parent_class ) . '-notice">' .
				'<div class="' . esc_attr( $parent_class ) . '-notice__inner">' .
					...
					'<button type="button" class="' . esc_attr( $parent_class ) . '-notice__button cmsmasters-button-spinner">' . esc_html__( 'Finish', 'bella-beauty' ) . '</button>' .
				'</div>' .
			'</div>' .
			...
			echo '<li class="' . esc_attr( $parent_class ) . '__item' . ( $active_demo === $demo_key ? ' cmsmasters-active' : '' ) . '">' .
				...
				'<div class="' . esc_attr( $parent_class ) . '__item-info">' .
					...
					'<div class="' . esc_attr( $parent_class ) . '__item-buttons">' .
						'<a href="' . esc_url( get_admin_url() . 'admin.php?page=cmsmasters-options-license' ) . '" class="button cmsmasters-button-spinner' . ( API_Requests::check_token_status() ? ' cmsmasters-demo-apply-button' : '' ) . '" data-demo-key="' . esc_attr( $demo_key ) . '">' . esc_html__( 'Apply', 'bella-beauty' ) . '</a>' .
	...
	public function ajax_apply_demo() {
		if ( ! check_ajax_referer( 'cmsmasters_options_nonce', 'nonce' ) ) {
			wp_send_json( array(
				'success' => false,
				'code' => 'invalid_nonce',
				'message' => esc_html__( 'Yikes! Demo activation failed. Please try again.', 'bella-beauty' ),
			) );
		}
		...
		if ( ! isset( $_POST['demo_key'] ) ) {
			wp_send_json( array(
				...
				'message' => esc_html__( 'Empty demo data.', 'bella-beauty' ),
			) );
		}
		...
		wp_send_json( array(
			...
			'message' => wp_kses_post(
				'<h3>' . __( 'New design is now applied!', 'bella-beauty' ) . '</h3>' .
				'<p>' . __( 'Click Finish and allow a few seconds after that for new appearance to be applied (do not reload your page).', 'bella-beauty' ) . '</p>' .
				( 'demos' === Theme_Config::IMPORT_TYPE ? '<p>' . sprintf(
					__( 'We recommend that you run a %1$sRegenerate Thumbnails%2$s tool to resize existing images for your new design.', 'bella-beauty' ),
	...
	public function page_info_notice() {
		...
		echo '<div class="notice notice-info">' .
			'<p><strong>' . esc_html__( 'Applying the design will change only the design system (Global Colors, Global Fonts settings) and Theme Settings.', 'bella-beauty' ) . '</strong></p>' .
			'<p>' .
				sprintf(
					__( 'You can read about this, along with more information and tips on switching design concepts, in our docs article - %1$sopen article%2$s.', 'bella-beauty' ),
