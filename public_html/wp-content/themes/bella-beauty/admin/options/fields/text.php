<?php
namespace BellaBeautySpace\Admin\Options\Fields;

use BellaBeautySpace\Admin\Options\Options_Utils;
	...
	public static function validate( $id, $value, $args ) {
		...
		switch ( $args['subtype'] ) {
			case 'numeric':
				...
				if ( false === is_numeric( $value ) ) {
					add_settings_error(
						...
						esc_html__( 'Expecting a Numeric value! Please fix.', 'bella-beauty' ),
						'error'
					);
				}
			...
			case 'multinumeric':
				...
				if ( '' !== $value ) {
					$out = ( 1 === preg_match( '/^-?\d+(?:,\s?-?\d+)*$/', $value ) ? $value : esc_html__( 'Expecting comma separated numeric values', 'bella-beauty' ) );
				} else {
					$out = $value;
				}
				...
				if ( '' !== $value && 1 !== preg_match( '/^-?\d+(?:,\s?-?\d+)*$/', $value ) ) {
					add_settings_error(
						...
						esc_html__( 'Expecting comma separated numeric values! Please fix.', 'bella-beauty' ),
						'error'
					);
				}
			...
			case 'email':
				...
				if ( '' !== $value ) {
					$out = ( false !== is_email( $value ) ? $value : esc_html__( 'Oops, looks like you made a mistake with the email address', 'bella-beauty' ) );
				} elseif ( '' === $value ) {
					$out = esc_html__( 'This setting field cannot be empty! Please enter a valid email address.', 'bella-beauty' );
				}
				...
				if ( false === is_email( $value ) || '' === $value ) {
					add_settings_error(
						...
						esc_html__( 'Please enter a valid email address.', 'bella-beauty' ),
						'error'
					);
				}
