...
class API_Requests {
	...
	public static function check_token_status() {
		return get_option( 'cmsmasters_bella-beauty_token_status', 'invalid' ) === 'valid';
	}
	...
	public static function get_request( $route, $args = array() ) {
		$cache_key = 'cmsmasters_bella-beauty_cached_request_' . $route . '_' . md5( serialize( $args ) );
	...
	public static function post_request( $route, $args = array() ) {
		...
		$response = wp_remote_post(
			...
			array(
				'headers' => array(
					'Authorization' => 'Bearer ' . get_option( 'cmsmasters_bella-beauty_token', 'invalid' ),
				),
				'body' => $args,
				'timeout' => 60,
			)
		...
		if ( 200 !== wp_remote_retrieve_response_code( $response ) ) {
			if ( 'regenerated' !== get_transient( 'cmsmasters_bella-beauty_token_regeneration_status' ) ) {
	...
	public static function regenerate_token( $die = false ) {
		if ( 'regenerated' === get_transient( 'cmsmasters_bella-beauty_token_regeneration_status' ) ) {
			...
			wp_send_json_error( esc_html__( 'Token not regenerated. The token was regenerated earlier.', 'bella-beauty' ), 403 );
		}
		...
		set_transient( 'cmsmasters_bella-beauty_token_regeneration_status', 'regenerated', 300 );

		if ( 200 !== $response_code ) {
			update_option( 'cmsmasters_bella-beauty_token_status', 'invalid' );
		...
		update_option( 'cmsmasters_bella-beauty_token', $response_body['data']['token'] );
		update_option( 'cmsmasters_bella-beauty_token_status', 'valid' );
	...
	public static function generate_token( $args = array() ) {
		...
		if ( ! empty( $args['user_email'] ) && false === is_email( $args['user_email'] ) ) {
			wp_send_json(
				array(
					...
					'code' => 'invalid_email',
					'error_field' => 'email',
					'message' => esc_html__( 'Oops, looks like you made a mistake with the email address', 'bella-beauty' ),
				)
			);
		}
		...
		update_option( 'cmsmasters_bella-beauty_token_data', $token_data );
		update_option( 'cmsmasters_bella-beauty_token', $response_body['data']['token'] );
	...
	public static function get_token_data( $die = true ) {
		$data = get_option( 'cmsmasters_bella-beauty_token_data', array() );
		...
		if (
			...
			wp_send_json( array(
				...
				'message' => esc_html__( 'Your token data is invalid.', 'bella-beauty' ),
			) );
		}

		update_option( 'cmsmasters_bella-beauty_token_data', $data );
	...
	protected static function delete_token_data() {
		...
		delete_option( 'cmsmasters_bella-beauty_token_data' );
		delete_option( 'cmsmasters_bella-beauty_token' );
		update_option( 'cmsmasters_bella-beauty_token_status', 'invalid' );
	}
